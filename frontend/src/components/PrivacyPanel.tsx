...
export default function PrivacyPanel({ auction }: { auction: Auction }) {
  ...
  const rows: { label: string; shown: boolean; note?: string }[] = [
    { label: 'Existencia de la subasta', shown: true },
    { label: 'Emisor (dirección)', shown: true },
    { label: 'Activo y monto emitido', shown: true },
    { label: 'Compromiso de cada oferta (hash)', shown: true },
    { label: 'Monto de cada oferta', shown: settled, note: settled ? 'revelado tras el cierre' : 'oculto hasta el cierre' },
    { label: 'Monto del pago al emisor', shown: false, note: 'compromiso Pedersen' },
  ]
